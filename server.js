...
function gateKeeper(req, res, next) {
  const userPwHead = req.get('x-username-and-password')
  const userPw = queryString.parse(userPwHead)
  const {user, pass} = userPw

  let userObj = {}

  const verifyUserObject = (USERS.find((individ) => {userObj = individ; return individ.userName == user}) != undefined) ? userObj : {} 
  
  const {password} = verifyUserObject 
    
  const verifyPass = ((password == pass) != false) ? req.user = userObj : {}
